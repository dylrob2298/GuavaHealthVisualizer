...
def load_data(path="transformed_guava.csv") -> pd.DataFrame:
    """
    Robustly load data from a CSV file.
    It prefers 'dd-mm-yyyy' but can handle other formats.
    """
    try:
        df = pd.read_csv(path)
    except FileNotFoundError:
        # This is expected if the app is run for the first time.
        # Return an empty DataFrame, the UI will prompt for an upload.
        return pd.DataFrame()

    # Robustly parse the 'Datetime' or 'Date' column
    if "Datetime" in df.columns:
        ...
        # If no date columns, there's nothing to plot.
        st.warning("Uploaded CSV must contain a 'Datetime' or 'Date' column.")
        return pd.DataFrame()
...
with st.sidebar.expander("Upload new data", expanded=True):
    ...
    xml_file = st.file_uploader(
        "Apple Health export.xml", type=["xml"], key="u_export_xml"
    )

    if st.button("Process and use uploaded data", type="primary", use_container_width=True):
        if not guava_file:
            st.warning("Please upload a Guava CSV file to process.")
        else:
    ...
    # --- This section now correctly formats the download button ---
    if not df.empty:
      transformed_for_download = df.copy()
      if "Date" in transformed_for_download.columns:
          transformed_for_download["Date"] = pd.to_datetime(
              transformed_for_download["Date"]
          ).dt.strftime("%d-%m-%Y")
      if "Datetime" in transformed_for_download.columns:
          transformed_for_download["Datetime"] = pd.to_datetime(
              transformed_for_download["Datetime"]
          ).dt.strftime("%d-%m-%Y %H:%M:%S")

      st.download_button(
          "Download processed CSV",
          data=transformed_for_download.to_csv(index=False),
...
if df.empty:
    st.sidebar.warning("Upload data to enable dashboard controls.")
    # Create an empty df to prevent the rest of the app from crashing
    d = pd.DataFrame()
else:
    ...
    date_range = st.sidebar.slider(
        ...
        min_value=date_min,
        max_value=date_max,
        format="DD-MM-YYYY", # Use Streamlit's format string
    )
    ...
    target_water = st.sidebar.number_input(
        "Water target (mL)", 0.0, 10000.0, 2000.0, 100.0
    )
    target_ex = st.sidebar.number_input(
        "Exercise target (minutes)", 0.0, 300.0, 30.0, 5.0
    )
    ...
    for c in ["Steps", "Exercise Minutes", "Sleep Hours", "Walking HR Avg", "Energy"]:
        if c in d.columns:
            d[f"{c} ({window}d MA)"] = (
                d[c].rolling(window, min_periods=1).mean().round(2)
            )
...
if d.empty:
    st.info("No data available for the selected date range. Please upload data or adjust the date range slider in the sidebar.")
else:
...
if num_cols:
    ...
    for tab, col in zip(ts_tabs, num_cols):
        with tab:
            ...
            hovertemplate = f"<b>%{{x|%d-%m-%Y}}</b><br>{col}: %{{y:.2f}}"
            for i, hover_col_name in enumerate(available_hover_cols):
                label = hover_col_name.replace(" (str)", "")
                formatter = ":.1f" if hover_col_name == "Energy" else ":.0f" if hover_col_name == "Total Symptoms" else ""
                hovertemplate += f"<br>{label}: %{{customdata[{i}]{formatter}}}"
            hovertemplate += "<extra></extra>"

            fig_ts.add_trace(go.Scatter(
                x=transformed["Datetime"], y=y, name=col, mode="lines+markers", marker=dict(size=5),
            ...
            if show_roll:
                ...
                if ma_col:
                    ...
                    fig_ts.add_trace(go.Scatter(
                        x=transformed["Datetime"], y=transformed[ma_col], name=ma_name,
                        mode="lines", line=dict(dash="dash"), hovertemplate=f"{ma_name}: %{{y:.2f}}<extra></extra>",
                    ))
            ...
            fig_ts.update_layout(
                title=f"{col} over time", hovermode="x unified", xaxis_title="Date", yaxis_title=y_title,
...
if not d.empty:
    ...
    options = num_cols
    if len(options) < 2:
        st.info("You need at least two numeric metric columns to use this comparison chart.")
    else:
        c1, c2 = st.columns(2)
        ...
        fig_dual.add_trace(go.Scatter(x=transformed["Datetime"], y=transformed[metric1], mode="lines", name=metric1, hovertemplate=f"{metric1}: %{{y:.2f}}<extra></extra>"), secondary_y=False)
        if show_roll:
            ma1_name = add_ma(transformed, metric1, win)
            if ma1_name:
                fig_dual.add_trace(go.Scatter(x=transformed["Datetime"], y=transformed[ma1_name], mode="lines", name=ma1_name, line=dict(dash="dash"), hovertemplate=f"{ma1_name}: %{{y:.2f}}<extra></extra>"), secondary_y=False)
        fig_dual.add_trace(go.Scatter(x=transformed["Datetime"], y=transformed[metric2], mode="lines", name=metric2, hovertemplate=f"{metric2}: %{{y:.2f}}<extra></extra>"), secondary_y=True)
        if show_roll:
            ma2_name = add_ma(transformed, metric2, win)
            if ma2_name:
                fig_dual.add_trace(go.Scatter(x=transformed["Datetime"], y=transformed[ma2_name], mode="lines", name=ma2_name, line=dict(dash="dash"), hovertemplate=f"{ma2_name}: %{{y:.2f}}<extra></extra>"), secondary_y=True)
        fig_dual.update_layout(title_text=f"{metric1} vs. {metric2} Over Time", hovermode="x unified", margin=dict(l=40, r=20, t=60, b=40), legend=dict(orientation="h", y=1.02, yanchor="bottom", xanchor="right", x=1))
        fig_dual.update_xaxes(title_text="Date", hoverformat="%d-%m-%Y")
    ...
    if not num_cols:
        ...
        left, right = st.columns(2)
        y_axis_metric = left.selectbox("Choose a metric to plot:", options=num_cols, index=num_cols.index("Energy") if "Energy" in num_cols else 0)
        symptom_options = sorted(list(pd.Series(np.concatenate(plot_df["Symptoms"].values)).dropna().unique())) if has["Symptoms"] else []
        aid_options = sorted(list(pd.Series(np.concatenate(plot_df["Mobility Aids"].values)).dropna().unique())) if has["Mobility Aids"] else []
        symptom_filter = []
        if has["Symptoms"] and symptom_options:
            symptom_filter = right.multiselect("Filter by symptoms:", options=symptom_options)
        mobility_aid_filter = []
        if has["Mobility Aids"] and aid_options:
            mobility_aid_filter = right.multiselect("Filter by mobility aids:", options=aid_options)
        if symptom_filter:
            plot_df = plot_df[plot_df["Symptoms"].apply(lambda s: bool(set(s) & set(symptom_filter)))]
        if mobility_aid_filter:
            plot_df = plot_df[plot_df["Mobility Aids"].apply(lambda a: bool(set(a) & set(mobility_aid_filter)))]
        if plot_df.empty:
            st.warning("No data matches the selected filters. Please broaden your criteria.")
        else:
            color_col = "Any Mobility Aid Used" if "Any Mobility Aid Used" in plot_df.columns else None
            plot_df["Symptoms (str)"] = plot_df["Symptoms"].apply(lambda s: ", ".join(map(str, s)) if (s and isinstance(s, list)) else "None")
            hover_data = {"Datetime": "|%d-%m-%Y", "Energy": ":.1f", "Total Symptoms": ":.0f", "Symptoms (str)": True, "Mobility Aids (str)": True}
            if y_axis_metric not in hover_data: hover_data[y_axis_metric] = ":.2f"
            fig_interactive = px.scatter(plot_df, x="Datetime", y=y_axis_metric, color=color_col, hover_data=hover_data, title=f"{y_axis_metric} Over Time (Filtered)")
            fig_interactive.update_traces(marker=dict(size=8, line=dict(width=1, color="DarkSlateGrey")))
            fig_interactive.update_layout(hovermode="x unified", xaxis_title="Date", yaxis_title=y_axis_metric, margin=dict(l=40, r=20, t=60, b=40), legend=dict(title="Mobility Aid Used"))
    ...
    if "Total Symptoms" in transformed.columns:
        ...
        if "Mobility Aids" in tmp.columns:
            tmp["Mobility Aids (list)"] = tmp["Mobility Aids"].apply(lambda v: ", ".join(map(str, v)) if v else "None")
        fig_sym_ct = px.bar(tmp, x="Datetime", y="Total Symptoms", color=color_col, custom_data=(["Mobility Aids (list)"] if "Mobility Aids (list)" in tmp.columns else None), title="Total Symptoms per Day" + (" (colored by mobility aid usage)" if color_col else ""))
        if "Mobility Aids (list)" in tmp.columns:
            fig_sym_ct.update_traces(hovertemplate="<b>%{x|%d-%m-%Y}</b><br>Total Symptoms: %{y}<br>Mobility Aids: %{customdata[0]}<extra></extra>")
    ...
    if "Symptoms" in transformed.columns:
        ...
        sym_counts = sym_long.dropna(subset=["Symptoms"]).groupby("Symptoms", as_index=False).size().sort_values("size", ascending=False)
        if not sym_counts.empty:
            fig_sym_freq = px.bar(sym_counts.head(int(top_n)), x="size", y="Symptoms", orientation="h", title=f"Top {int(top_n)} Recorded Symptoms", labels={"size": "Days observed", "Symptom": "Symptom"})
            fig_sym_freq.update_layout(yaxis={"categoryorder": "total ascending"}, margin=dict(l=40, r=20, t=60, b=40))
